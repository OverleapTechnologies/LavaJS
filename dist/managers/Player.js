...
const { newTrack, searchTrack, newPlaylist } = require("../utils/Utils");
class Player {
  /**
  ...
  lavaSearch(query, add = true, user) {
    return new Promise(async (resolve, reject) => {
      ...
      const { loadType, playlistInfo, tracks, exception } = await fetch(
      ...
      switch (loadType) {
        ...
        case "TRACK_LOADED":
          ...
          break;
        case "SEARCH_RESULT":
          const tracksData = searchTrack(tracks, user);
          resolve(tracksData);
          break;
        case "PLAYLIST_LOADED":
          const playlist = newPlaylist(playlistInfo, tracks, user);
          resolve(playlist);
          break;
        // Error loading
        case "NO_MATCHES":
