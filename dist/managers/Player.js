...
class Player {
    ...
    lavaSearch(query, user, add = true) {
        return new Promise(async (resolve, reject) => {
            const search = new RegExp(/^https?:\/\//g).test(query)
                ? query
                : `ytsearch:${query}`;
            const { loadType, playlistInfo, tracks, exception } = await (await fetch(`http://${this.node.options.host}:${this.node.options.port}/loadtracks?identifier=${search}`, {
    ...
    seek(position) {
        ...
        if (position < 0 || position > this.queue[0].length)
            throw new RangeError(`Player#seek() The provided position must be in between 0 and ${this.queue[0].length}.`);
    ...
    destroy() {
        ...
        this.lavaJS.playerCollection.delete(this.options.guild.id || this.options.guild);
